...
// Modificado: se agregó handleDeleteReservation y botón "Eliminar" cuando la reserva esté cancelada
...
export default function ProfileDashboard() {
  ...
  // 🔹 NUEVO: eliminar reserva cancelada
  ...
  return (
    <main className="min-h-screen flex flex-col items-center bg-[var(--background)] px-4 py-8">

      {/* Perfil */}
      <section className="card flex flex-col sm:flex-row items-center gap-6 p-6 mb-8 w-full max-w-4xl">
        <div className="flex flex-col sm:flex-row items-center gap-6">
          <div className="relative w-24 h-24 rounded-full overflow-hidden border-4 border-[var(--primary)] flex items-center justify-center">
            {user?.profilePicture ? (
              <Image src={user.profilePicture} alt={user.name} fill className="object-cover" />
            ) : (
              <User size={48} />
      ...
      {/* Tabs */}
      <div className="flex flex-wrap justify-center gap-3 my-6 w-full max-w-4xl bg-[var(--secondary)] border rounded-3xl p-3">
        {tabs.map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key as any)}
      ...
      <section className="card p-6 w-full max-w-4xl">
        {activeTab === 'reservations' && (
          <div>
            <h3 className="text-xl mb-4 font-semibold">Mis Reservas</h3>
