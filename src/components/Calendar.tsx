...
// const CalendarView = ({ onSelectClass }: CalendarViewProps) => {
//   const [currentWeek, setCurrentWeek] = useState(0);

//   return (
//     <div className="p-8 md:p-12 bg-(--secondary) border border-(--muted-foreground) rounded-2xl shadow-lg max-w-[1600px] mx-auto">
//       {/* Header */}
...
//                     className="w-full text-left p-4 md:p-5 rounded-2xl border border-(--muted-foreground) hover:border-(--primary) transition-colors bg-(--secondary) hover:bg-(--primary)/10"
//                   >
...
//                         />
//                       </div>
//                       <p className="text-[11px] md:text-xs text-[var(--muted-foreground)]">{spotsLeft}</p>
//                     </div>
//                   </button>
//                 );
//               })}
...
const CalendarView = ({ onSelectClass }: CalendarViewProps) => {
  ...
  return (
    <div className="p-6 md:p-12 bg-[var(--secondary)] border border-[var(--muted-foreground)] rounded-2xl shadowd-lg  my-6d mx-6">
      {/* Header */}
      <div className="flex items-center justify-between mb-8 px-2 md:px-6">
        <h3 className="text-2xl md:text-3xl font-semibold text-[var(--primary)]">Calendario Semanal</h3>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-5 px-2 md:px-6">
        {weekDays.map((day) => (
          <div key={day} className="space-y-3">
            {/* Día */}
            <div className="text-center pb-3 border-b border-[var(--muted-foreground)]">
              <p className="text-sm md:text-base font-semibold text-[var(--primary)]">{day}</p>
            </div>
              ...
              {scheduleData[day]?.map((classData) => {
                ...
                return (
                  <button
                    ...
                    className="w-full text-left p-4 md:p-5 rounded-2xl border border-[var(--muted-foreground)] hover:border-[var(--primary)] transition-colors bg-[var(--secondary)] hover:bg-[var(--primary)]/10"
                  >
                    <p className="text-base md:text-lg font-medium truncate text-[var(--card-foreground)]">{classData.name}</p>
                    <p className="text-sm text-[var(--muted-foreground)]">{classData.time}</p>

                    <div className="mt-3 flex items-center gap-2">
                      <div className="h-2 flex-1 bg-[var(--muted-foreground)] rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full ${isAlmostFull ? "bg-yellow-500" : "bg-[var(--primary)]"}`}
                          style={{ width: `${(classData.booked / classData.capacity) * 100}%` }}
                        />
                      </div>
                      <p className="text-[11px] md:text-xs text-[var(--muted-foreground)]">{spotsLeft}</p>
