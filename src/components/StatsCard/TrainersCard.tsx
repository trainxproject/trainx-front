...
import { getAllTrainers } from "@/services/trainersService";
import { Trainers } from "@/interfaces/Trainer";
import { createTrainer, rateTrainer } from "@/services/adminServices";
...
const TrainersCard: React.FC = () =>  {
    ...
    const handlerRate = async (trainerId: string) => {
        try {
            const updated = await rateTrainer(String(trainerId), ratingValue);

            if (updated) {
                const fresh = await getAllTrainers();
                setTrainers(fresh);
            }
        } catch (error) {
    ...
    return (
        <div className="min-h-screen flex flex-col bg-(--background) flex-wrap mt-29">
                ...
                {trainers.map((t) => (
                    ...
                    >
                        ...
                        <div className="flex items-center">
                            <button
                                onClick={() => setIconDelete(true)}
                                className="ml-auto p-2 rounded-md text-gray-400 hover:text-red-500 hover:bg-transparent transition"
                            >
                                <Trash2 size={18} />
                            </button>
                        </div>
            ...
            {iconDelete && (
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50">
                    <div className="bg-[hsl(var(--secondary))] border border-white/10 rounded-2xl p-8 max-w-md w-11/12 text-center">
                        <h2 className="text-2xl text-white">Eliminar Entrenador</h2>
            ...
            {modal && (
                <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
                    <div
                        className="rounded-2xl p-8 w-11/12 max-w-md relative"
                        style={{ background: "rgba(70, 70, 70, 1)" }}
                        onClick={(e) => e.stopPropagation()}
                    >
                        <button
                            onClick={() => setModal(false)}
                        ...
                        <form className="flex flex-col gap-5" onSubmit={handlerCreate}>
                            ...
                            <button className="mt-2 bg-white text-black font-medium px-4 py-2 rounded-lg hover:bg-gray-300">
                                Agregar
                            </button>
                        </form>
                    </div>
